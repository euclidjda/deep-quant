...
class DeepBayesUQModel(BaseModel):
    """
    ...
    def __init__(self, config):
        ...
        self._p_w = precision_w = tf.get_variable("precision_w", [num_hidden, num_outputs])
        precision_b = tf.get_variable("precision_b", [num_outputs])

        self._outputs = list()
        self._precision = list()
        for i in range(max_unrollings):
            output = tf.nn.xw_plus_b(rnn_outputs[i], output_w, output_b)
            precision = tf.nn.xw_plus_b(rnn_outputs[i], precision_w, precision_b)
        ...
        precisions = tf.concat(self._precision, 1)
        ...
        reversed_precisions = tf.reverse_sequence(precisions,
                                               self._seq_lengths, seq_axis=1, batch_axis=0)
        ...
        last_precision = tf.unstack(reversed_precisions, axis=1)[0]
        ...
        self._lp = last_precision
        self._lkt = last_k_targets
        self._lko = last_k_outputs
        self._lkp = last_k_precisions
        self._lkti = last_k_targets[:, :, ktidx]
        self._lkoi = last_k_outputs[:, :, ktidx]
        self._lkpi = last_k_precisions[:, :, ktidx]
        self._t = targets
        self._o = outputs
        self._p = precisions

        # Different components of mse definitions with precision
        if config.backfill is True:
            # TODO: Check if self._mean_squared_error_w_precision can be used here
            ...
            self._mse_0 = self._mean_squared_error_w_precision(last_k_targets[:, :, ktidx],
                                                   last_k_outputs[:, :, ktidx],
                                                   last_k_precisions[:, :, ktidx],
                                                   last_k_seqmask[:, :, ktidx])
            self._mse_1 = self._mean_squared_error_w_precision(last_k_targets, last_k_outputs,
                                                               last_k_precisions, last_k_seqmask)
