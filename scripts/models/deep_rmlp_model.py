...
class DeepRmlpModel(BaseModel):
    """
    A Deep Rmlp Model that supports regression with 
    ...
    def __init__(self, config):
        ...
        rnn_outputs, state = tf.contrib.rnn.static_rnn(stacked_rnn,
                                                       self._scaled_inputs[:num_outputs],
                                                       dtype=tf.float32,
                                                       sequence_length=self._seq_lengths)

        self._w = rnn_output_w = tf.get_variable("rnn_output_w", [num_hidden, num_outputs])
        rnn_output_b = tf.get_variable("rnn_output_b", [num_outputs])

        self._outputs = list()
        for i in range(max_unrollings-1):
            output = tf.nn.xw_plus_b( rnn_outputs[i], rnn_output_w, rnn_output_b )
        ...
        aux_inputs = self._scaled_inputs[-1][:,num_outputs:]
        mlp_output = tf.concat([rnn_output, aux_inputs], 1)
