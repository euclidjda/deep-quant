...
class DeepMlpUQModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        self._mse_var = self._mean_squared_error_w_variance(targets[:, ktidx],
                                                            outputs[:, ktidx],
                                                            variances[:, ktidx],
                                                            config)
        self._mse_var_all = self._mean_squared_error_w_variance(targets, outputs, variances, config)
        ...
        if config.data_scaler is not None and config.scale_targets is True:
            ...
            self._predictions_var = self._reverse_center_and_scale(variances)
        else:
            self._predictions = outputs
            self._predictions_var = variances

        # from here down is the learning part of the graph
        L = config.target_lambda
        # loss = L * self._mse + (1.0 - L) * self._mse_all
        loss = L * self._mse_var + (1.0 - L) * self._mse_var_all
    ...
    def _mean_squared_error_w_variance(self, targets, outputs, variances, config):
        ...
        loss = tf.multiply(diff, 1. / variances) + tf.log(variances)
        # TODO: Make the below safe to div by zero
        mse_v = tf.reduce_mean(loss)
        return mse_v
