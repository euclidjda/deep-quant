...
for my $model_type ('DeepRnnModel') {

    #for my $scaler ('StandardScaler','RobustScaler') {
    for my $scaler ('RobustScaler') {

	#for my $optimizer ('AdadeltaOptimizer','AdagradOptimizer','GradientDescentOptimizer') {
	for my $optimizer ('AdadeltaOptimizer') {

	    for my $rnn_cell ('gru','lstm') {

		for my $input_dropout ('True','False') {

		    for my $hidden_dropout ('True','False') {

			my @rnn_dropouts = ('False');
			
			push @rnn_dropouts, 'True' if $rnn_cell eq 'lstm';

			for my $rnn_dropout (@rnn_dropouts) {
				...
				for my $rnn_lambda (@rnn_lambdas) {
					...
					for my $num_hidden (@hidden_list) {
						...
						if (($input_dropout eq 'True') || ($hidden_dropout eq 'True') || ($rnn_dropout eq 'True'));
						...
						for my $init_scale (0.01,0.1) {
							...
							my $name = sprintf("%s-%s-%s-%s-l%d-h%04d-t%03d-r%02d-k%02d-i%02d-m%03d-%s%s%s",
									   substr(lc($model_type),4,3),
									   substr(lc($optimizer),0,4),
									   substr(lc($scaler),0,4),
									   substr(lc($rnn_cell),0,3),
									   $num_layers,
									   $num_hidden,
